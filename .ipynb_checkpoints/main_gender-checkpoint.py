...
if __name__ == '__main__':
    ...
    print(f"pheno={args['pheno']}, model={args['model']}, lr={args['lrate']}, seed={args['seed']}, batsize={args['batch']}, imgsize={args['imshape']}, \
    ...
    mwrapper = Modelwrapper(num_classes=2)
    cmodel = getattr(mwrapper, model_name)().to(device)

    criterion = nn.CrossEntropyLoss()
    optimizer_ft = optim.Adam(cmodel.parameters(), lr=lr)
    # Decay LR by a factor of 0.5 every 5 epochs
    stepdivid = 7 if num_epochs>14 else 5
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=round(num_epochs/stepdivid), gamma=0.7)

    model_args = {"model": cmodel, "criterion": criterion, "optimizer": optimizer_ft, "scheduler": exp_lr_scheduler,
                           "num_epochs": num_epochs, "dataloaders": dataloaders, "dataset_sizes": dataset_sizes, 'model_name': model_name}
    [model_ft, best_val_acc] = train_model(**model_args)
