...
class ImgDataset_withimagename(Dataset):
    ...
    def __init__(self, csv_file, root_dir,
        ...
        """
        self.labels = pd.read_csv(csv_file)
        if resample > 0 and resample <100: self.labels = self.labels.sample(frac=resample/100)
        self.root_dir = root_dir
        self.transform = transform
...
class ImgDataset_withaugment(Dataset):
    ...
    def __init__(self, csv_file, root_dir, resample=0, rodegree=0, randcolor=False, crop=False):
        """
        Args:
            csv_file (string): Path to the csv file with annotations.
            root_dir (string): Directory with all the images.
